...
export default function Navbar() {
  ...
  return (
    <nav className="bg-white border-b border-slate-200 px-6 py-3 flex items-center justify-between">
      <Link to="/" className="font-bold text-brand-700 text-lg">EEC Student Document Portal</Link>
      <div className="flex items-center gap-4 text-sm">
        {user && <Link to="/student/dashboard" className="hover:text-brand-600">Dashboard</Link>}
        {user ? (
          <>
